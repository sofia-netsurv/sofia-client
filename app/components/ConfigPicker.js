...
export default function ConfigPicker(props) {
  ...
  function setInfo() {
    ...
    pyshell.send(JSON.stringify(config))
  }
  }
  return (
    <form autoComplete="off">
      <FormControl className={classes.formControl}>
        <InputLabel htmlFor="demo-controlled-open-select">{props.configLabel}</InputLabel>
        <Select
          open={open}
          onClose={handleClose}
          onOpen={handleOpen}
        ...
        >
          {props.configOptions.map((item,i) => <MenuItem value={item.value} key={i}>{item.label}</MenuItem>)}
